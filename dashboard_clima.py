...
st.markdown(
    ...
    <div style="text-align: center; margin-top: -10px; margin-bottom: 15px;">
        ...
        <div style="color: #00f2ff; font-size: 1.6rem; font-weight: bold; text-shadow: 0 0 10px #00f2ff, 0 0 20px #00f2ff; margin-bottom: 12px;">
            José Esquina
        </div>
        <a href="https://www.linkedin.com/in/jose-esquina-0350aa159" target="_blank" style="
            text-decoration: none; color: white; border: 1px solid #00f2ff; padding: 6px 20px; border-radius: 25px; font-weight: bold; display: inline-block; margin-bottom: 12px;">
...
# -----------------------------------------------------------------------------
# 2. GESTIÓN DE ESTADO Y CARGA DE DATOS (SIN CAMBIOS)
# -----------------------------------------------------------------------------
...
def cargar_datos():
    ...
    for col in cols_no_cero: df_final.loc[df_final[col] == 0, col] = np.nan
...
if st.sidebar.button("🧹 RESTAURAR TODO"): reset_filtros(); st.rerun()
...
if estacion_selec != st.session_state.estado_estacion:
    st.session_state.estado_estacion = estacion_selec; st.rerun()
...
mes_inicio_num, mes_fin_num = ORDEN_MESES.index(mes_inicio) + 1, ORDEN_MESES.index(mes_fin) + 1
...
if mes_inicio_num <= mes_fin_num: mask &= (df['Mes_Num'] >= mes_inicio_num) & (df['Mes_Num'] <= mes_fin_num)
df_filtrado = df[mask].copy()

# -----------------------------------------------------------------------------
# 6. FUNCIONES DE GRAFICACIÓN (CORREGIDAS)
# -----------------------------------------------------------------------------
...
def plot_linea_mensual(data, x, y, titulo, color_hex):
    # Gráfica de línea limpia, agrupada por nombre de mes
    fig = px.line(data, x=x, y=y, title=titulo, markers=True, color_discrete_sequence=[color_hex], 
                  template='plotly_dark', category_orders={x: ORDEN_MESES},
                  labels={x: "Mes", y: "Temp Media (°C)"})
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(t=40, b=20))
    return fig
...
with tab_resumen:
    ...
    header_text = f"📡 ESTACIÓN: {estacion_selec} | 📅 AÑO: {anios_str}"
    ...
    if df_filtrado.empty:
        ...
        with col_g2:
            ...
            df_temp_m = df_filtrado.groupby(['Mes_Nombre'], observed=False)['Temp_Media'].mean().reset_index()
            st.plotly_chart(plot_linea_mensual(df_temp_m, 'Mes_Nombre', 'Temp_Media', "🌡️ Promedio Mensual de Temperatura", "#ffe600"), use_container_width=True)

        # Mapa y Tabla 
        st.markdown("---")
        st.subheader("📍 Ubicación Geográfica")
        df_mapa = df_filtrado.groupby(['NOMBRE_ESTACIÓN', 'Latitud', 'Longitud']).agg({'Precipitacion': 'sum'}).reset_index()
        fig_map = px.scatter_mapbox(df_mapa, lat="Latitud", lon="Longitud", hover_name="NOMBRE_ESTACIÓN", zoom=6, mapbox_style="carto-positron", height=400)
